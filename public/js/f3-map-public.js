var f3_main_function = (function ($) {
  ...
  function setupMap(selector, pos) {
    ...
    var data = [];
    // select the <tr> elements, and iterate over them finding the child 
    // elements that need to be setup with the correct data.
    ...
    $('.f3-table-cell-marker').click(function() {
      // find the parent <tr> with a class of .ao-location so that we
      // can find the data that we need to work with.
      var parent = $(this).parent('.ao-location');
      if (!parent) return;
  ...
  function mapao(data) {
    ...
    marker.addListener('click', function(evt) {
      ...
      $('#f3-map-details-location').text(data.location);
  ...
  // expose the main entry point globally. This is probably not a good idea and should be refactored soon! 
